...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public Cursor getAllLines() {
        Cursor cursor = this.getReadableDatabase().query(TABLE_LINES,
                new String[]{COLUMN_LINE_NETWORK, COLUMN_LINE_ID},
    ...
    public Cursor getElevators(String stationName) {
        return getReadableDatabase().query(
                TABLE_ELEVATORS,
                new String[]{COLUMN_ELEVATOR_ID,
                        COLUMN_ELEVATOR_SITUATION,
                        COLUMN_ELEVATOR_DIRECTION,
                        COLUMN_ELEVATOR_STATUS_DESC,
                        COLUMN_ELEVATOR_STATUS_TIME},
                new StringBuilder().append(COLUMN_ELEVATOR_STATION).append("=?")
    ...
    public boolean addLineToDatabase(String id, String network) {
        ...
        return getWritableDatabase().insertWithOnConflict(TABLE_LINES, null, values,
                SQLiteDatabase.CONFLICT_REPLACE) != -1;
    }

    public boolean deleteLineFromDatabase(String id) {
        return getWritableDatabase().delete(TABLE_LINES, COLUMN_LINE_ID + " =?",
                new String[]{id}) != 0;
    }
    ...
    public boolean deleteStationFromDatabase(String name) {
        getWritableDatabase().delete(TABLE_LINES_STATIONS, COLUMN_STATION_NAME
                + " =?", new String[]{name});
        getWritableDatabase().delete(TABLE_STATIONS, COLUMN_STATION_NAME + " =?",
                new String[]{name});
        return true;
    }
    ...
    public boolean deleteElevatorFromDatabase(String id) {
        return getWritableDatabase().delete(TABLE_ELEVATORS, COLUMN_ELEVATOR_ID
                + " =?", new String[]{id}) != 0;
    }
